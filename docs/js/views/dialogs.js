...
/**
 * Secondary/disclosure UI: settings (subscriptions + LLM key), profile &
 * context (You.md + watch-history CSV), and the trace/export/catalog-info
 * panel. Kept out of the primary chat + recommendation view so they don't
 * compete for attention.
 */
export function createDialogs(el, deps) {
  let parsedHistoryDraft; // undefined = no pending change this dialog session
  const webSearch = el.llmWebSearch || document.getElementById("llm-web-search");
  ...
  function updateWebSearchAvailability() {
    const available = isOpenRouterUrl(el.llmBaseUrl.value.trim());
  ...
  function wireSettings() {
    ...
    el.settingsSave.addEventListener("click", async () => {
      ...
      const llm = {
        ...
        webSearch: isOpenRouterUrl(el.llmBaseUrl.value.trim()) && webSearch.checked === true
      };
    ...
    el.llmBaseUrl.addEventListener("input", updateWebSearchAvailability);
  }

  // ---- Profile & context: You.md + watch-history CSV ----
  ...
  function wireContext() {
    el.contextBtn.addEventListener("click", async () => {
      try {
        ...
        el.historySummary.textContent = history ? deps.summarizeHistory(history) : "No history imported.";
    ...
    el.historyFile.addEventListener("change", () => {
      ...
      deps.readCsvFile(file, (parsed, summaryText) => {
        if (parsed) parsedHistoryDraft = parsed;
        el.historySummary.textContent = summaryText;
      });
    });
  ...
  // ---- Disclosure: agent trace, exports, catalog provenance ----
  ...
  function wireDisclosure() {
    el.disclosureBtn.addEventListener("click", () => {
      fillCatalogDetail(deps.catalogMeta());
      feedback(el.disclosureFeedback);
      el.disclosureDialog.showModal();
    });
