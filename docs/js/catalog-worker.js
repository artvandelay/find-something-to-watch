...
function toScope(payload) {
  ...
  const subscriptions = Array.isArray(scope.subscriptions)
    ? scope.subscriptions
    : Array.isArray(scope.providers) ? scope.providers : [];
  return {
    subscriptions: new Set(subscriptions.filter((value) => typeof value === "string")),
    provider: typeof scope.filters?.provider === "string" ? scope.filters.provider : null,
    lang: typeof scope.filters?.lang === "string" ? scope.filters.lang : null,
    genre: typeof scope.filters?.genre === "string" ? scope.filters.genre : null
  };
}

function scopeFilters(payload) {
  const input = scopeInput(payload);
  const source = input.filters && typeof input.filters === "object" ? input.filters : {};
  const scope = toScope(payload);
  return {
    ...source,
    excludeKeys: Array.isArray(payload?.excludeKeys) ? payload.excludeKeys : source.excludeKeys,
    providers: scope.subscriptions,
    provider: scope.provider || source.provider,
    lang: scope.lang || source.lang,
    genre: scope.genre || source.genre
  };
}
...
function describe(payload) {
  ...
  const manifest = {
    ...
    filters: { provider: filters.provider || null, lang: filters.lang || null, genre: filters.genre || null }
  };
    ...
    executor.postMessage({
      ...
      projectionJson: JSON.stringify({
        ...
        context: {
          subscriptions: [...scope.subscriptions],
          filters: {
            provider: scopeFilters(payload).provider || null,
            lang: scopeFilters(payload).lang || null,
            genre: scopeFilters(payload).genre || null
          }
        }
      })
    });
